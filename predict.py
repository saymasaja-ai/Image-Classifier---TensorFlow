...
def predict(image_path, model, class_names, top_k=5):
    ...
    top_k_indices = probs.argsort()[-top_k:][::-1]  # Top K indices
    top_probs = probs[top_k_indices]
    top_labels = [class_names[str(i+1)] for i in top_k_indices]  # Map labels using JSON
...
if __name__ == "__main__":
    ...
    #  Load Keras model (not TFSMLayer!)
    ...
    # Make prediction
    probs, labels = predict(args.image_path, model, class_names, args.top_k)
